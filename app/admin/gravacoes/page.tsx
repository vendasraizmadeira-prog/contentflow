    ...
    <div className="p-6 max-w-3xl">
      ...
      {loading ? (
        <div className="flex flex-col gap-3">
          {[1, 2, 3].map(i => <div key={i} className="rounded-2xl animate-pulse" style={{ background: "#0F0F1E", height: 72 }} />)}
        </div>
      ) : recordings.length === 0 ? (
        ...
        <div className="flex flex-col gap-3">
          {recordings.map(r => {
            ...
            return (
              <div key={r.id} className="rounded-2xl p-5 flex items-center gap-4" style={{ background: "#0F0F1E", border: "1px solid #22223A" }}>
            ...
            <div className="flex flex-col gap-4 mb-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="date" value={form.date} onChange={e => setForm({ ...form, date: e.target.value })} className="w-full px-4 py-3 rounded-xl text-sm outline-none" style={{ background: "#0B0B0F", border: "1px solid #22223A", color: "#fff" }}/>
                </div>
                <div>
              ...
              <div>
                ...
                <input value={form.location} onChange={e => setForm({ ...form, location: e.target.value })} placeholder="Ex: Estúdio A, local do cliente..." className="w-full px-4 py-3 rounded-xl text-sm outline-none" style={{ background: "#0B0B0F", border: "1px solid #22223A", color: "#fff" }}/>
              </div>
            </div>
