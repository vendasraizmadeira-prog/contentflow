...
export default function AdminBottomNav() {
  ...
  return (
    <nav
      ...
      style={{ background: "#0F0F17", borderTop: "1px solid #1E1E2A", paddingBottom: "env(safe-area-inset-bottom)" }}
    >
      {/* Selected client strip */}
      {selectedClient && (
        <div className="flex items-center gap-2 px-4 py-2 border-b" style={{ borderColor: "#1E1E2A", background: "rgba(123,77,255,0.08)" }}>
          ...
          return (
            <Link key={item.href} href={item.href}>
              <div className="flex flex-col items-center gap-1 px-3 py-1">
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={active ? "#7B4DFF" : "#6B7280"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <path d={item.icon} />
                </svg>
                <span className="text-xs" style={{ color: active ? "#7B4DFF" : "#6B7280", fontSize: 10 }}>
                  {item.label}
                </span>
              </div>
            </Link>
